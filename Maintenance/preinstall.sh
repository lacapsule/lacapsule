#!/bin/bash

printf "\e[8;50;53t" &&
echo "
█████████████████████████████████████████████████████
█████████████████████▛▛▛▛▟▙▙▙▙▙▙▌▀▘        ▀▀▜███████
█ ꜱᴄʀɪᴘᴛ ᴅᴇ █████▛▛▙▙▜▜▜▜▙▙▙▙▙▛▟            ▄▄▄▛█████
███████████████▜▚▛▛▟▞▛▛▛▙▙▙▙▙▙█▙           █▛█▜█▜████
█████████████▜▞▛▙▜▜▚▛▛▛█▟▟▟▟▟▚▙▛▖          ▛███▜█▜███
███████████▛▙▜▞▛▟▜▚▛▛▛▛▟▟▟▟▟▟▜▙▛▙          ▜█▟▛███▜▞█
██████████▜▟▞▙▜▜▞▙▛▛▟▜▜▚▙▙▙▙▛█▞▛▙▖          ▚███▟▛█▌▜
█████████▜▚▚▜▞▛▟▜▟▞▛▙▛▛█▟▟▟▟▜▟▜▛█▟▖          ▀▟▛▙██ ▐
████████▙▜▞▛▙▜▞▙▙▙▜▜▞▛▛▟▟▟▟▟▜▟▜▜▟▚▛▖           ▝▀▀   
███████▘▚▙▜▟▞▙▜▟▞▟▜▚▛▛▛▙▙▙▙▛█▟▛█▟▛▛▙▖               
██████▛▝▐▞▙▚▜▞▙▙▜▞▛▛▟▜▜▟▟▟▞█▟▟▜▙▙▛█▜▟▄              ▐
██████▘▚ ▛▞▛▙▜▞▞▙▛▛▟▜▜▙▙▙▙▛▙▙▛▙▙▛▛▙▛▙▛▌▖            ▟
█████▛▝▗▝▐▜▟▞▙▜▜▟▞▛▛▛▙▙▙▙▙▛▙▙▛█▟▜▜▙█▙▛█▜▄          ▞█
█████▌▚▗▝ ▙▚▜▞▛▙▙▜▜▜▟▚▙▙▙▙▛▙▛▛▙▛▛█▟▟▞█▟▜▟▟▄       ▞▛█
█████▘▖▗▝▗▝▛▙▜▟▞▟▜▚▛▟▜▟▟▟▞▛▙▛█▙▛█▚▙▙▛▙▛█▟▟▟▟▄▖ ▗▗▙▜▜█
█████▗ ▘▖▘▖▞▟▚▙▜▞▙▛▛▙█▟▟▞▛▛▙█▙▙▛▛▛▙▛▛▙█▟▟▟▟▟▟▟▜▟▜▟▜▜█
█████▗▝▗▗▗▗▝▝▙▙▜▜▞▛▟▙▙▙▙▛▛█▜▟▟▟▜▛█▙▛▛▙▙▙▙▙▙▙▙▜▜▟▜▟▜██
████▌▘▗▝ ▖▝▗▚▗▘▌▞▛▙▙▜▟▟▞█▜▟▟▙█▙█▜▟▟▜▟▜▜▟▚▙▙▙▛▛█▞█▞███
████▌▘▖▗▝▝▗▝▐▝▖▖▌▞▞▞▜▚▛▙▜▜▟▜▙▛▟▜▟▟▜▜▟▜▟▛▟▜▙▙█▞█▞▛▟███
████▌▖▗ ▖▘▗▝▖▌▚▚▐▝▞▞▌▌▀▀▜▜▟▜▟▜▜▜▟▜▜▜▟▛▙▛▙█▟▟▞█▞██████
████ ▖▗ ▖▗▗▝▞▖▚▝▖▌▞▖▌▚▚▘▌▌▌▌▌▞▞▞▞▗▘▀▀▀▗▗ ▖ ▗▝▟███████
████ ▖ ▖▗ ▗▚▗▐▝▞▝▖▚▐▝▞ ▘▘▘▘▗▗▗▗▗▗▗▝ ▘▖▖▖▘▖▚▄█████████
███▌▖ ▘ ▖▝▖▞▖▘▚▝▝▝▗ ▖ ▘▘▝ ▘▖▗ ▖▗ ▖▝▝▗▗▗▗▙▟███████████
███▌ ▖▖▘ ▖▞ ▝▝ ▖▘▝ ▖ ▘▝ ▘▘▘▗▗▗▗▗▗ ▚▐▄▟███████████████
███ ▖  ▗ ▖ ▝ ▖▘ ▖▘▗ ▘▘▘▘▝▗▝ ▖▗▗▄▄████████████████████
██▛ ▗▝ ▖ ▗▝ ▘ ▗ ▗▝ ▗▗▗▗▝▗▄▄▙█████████████████████████
██▌▝  ▗ ▝  ▖ ▘▗▝ ▗▟▄▄▟███████████████████████████████
██  ▖▝  ▖ ▘ ▙▟▟██████████████████████████████████████
█▌ ▘  ▖▙▄███████████████████████ ʟᴀᴄᴀᴘꜱᴜʟᴇ.ᴏʀɢ ██████
█  ▄▄████████████████████████████████████████████████
▙▟███████████████████████████████████████████████████" &&
echo "" &&
sleep 1 &&

echo "" 
echo "         $(tput setaf 0)$(tput setab 9) Début du script d'installation... $(tput sgr0)" &&
sleep 1 &&

echo ""
echo ""
echo ""
echo " $(tput setaf 0)$(tput setab 9) Veuillez entrer votre mot de passe administrateur $(tput sgr0)" &&
echo ""
sleep 1 &&

sudo chown $USER -R /var/lib/dpkg/*
sudo chown $USER -R /var/cache/apt/*
sudo chmod +x ~/maintenance/phoronix.sh
sudo chmod +x ~/maintenance/phoronix.deb
sudo chmod +x ~/maintenance/phoronix.deb

echo ""
echo "        $(tput setaf 0)$(tput setab 9) Mise à jour de la liste des paquets $(tput sgr0)" &&
sleep 1 &&
echo ""

sudo apt update

echo ""
echo "                      ✅ OK ✅" &&
sleep 0.5 &&

echo ""
echo "         $(tput setaf 0)$(tput setab 9) Installation des nouveaux paquets $(tput sgr0)" &&
echo ""
sleep 1 &&

sudo apt upgrade -y

echo ""
echo "                      ✅ OK ✅" &&
sleep 0.5 &&

echo ""
echo "          $(tput setaf 0)$(tput setab 9) Installation de l'éditeur GEDIT $(tput sgr0)" &&
echo ""
sleep 1 &&

sudo apt install gedit -y

echo ""
echo "                      ✅ OK ✅" &&
sleep 0.5 &&

echo ""
echo "     $(tput setaf 0)$(tput setab 9) Installation de l'outil de versioning GIT $(tput sgr0)" &&
echo ""
sleep 1 &&

sudo apt install gnome-terminal -y

echo ""
echo "                      ✅ OK ✅" &&
sleep 0.5 &&

echo ""
echo "     $(tput setaf 0)$(tput setab 9) Installation de l'outil de monitoring HTOP $(tput sgr0)" &&
echo ""
sleep 1 &&

sudo apt install htop -y &&

echo ""
echo "                      ✅ OK ✅" &&
sleep 0.5 &&

echo ""
echo "           $(tput setaf 0)$(tput setab 9) Installation des dependances $(tput sgr0)" &&
echo ""
sleep 1 &&

sudo apt install hardinfo -y

echo ""
echo "                      ✅ OK ✅" &&
sleep 0.5 &&

echo ""
echo "           $(tput setaf 0)$(tput setab 9) Installation de Smartmontools $(tput sgr0)" &&
echo ""
sleep 1 &&

sudo apt install smartmontools -y

echo ""
echo "                      ✅ OK ✅" &&
sleep 0.5 &&

echo ""
echo "             $(tput setaf 0)$(tput setab 9) Installation de Screentest $(tput sgr0)" &&
echo ""
sleep 1 &&

sudo apt install screentest -y

echo ""
echo "                      ✅ OK ✅" &&
sleep 0.5 &&

echo ""
echo "               $(tput setaf 0)$(tput setab 9) Installation de Gzip $(tput sgr0)" &&
echo ""
sleep 1 &&

sudo apt install gzip -y

echo ""
echo "                      ✅ OK ✅" &&
sleep 0.5 &&

echo ""
echo "               $(tput setaf 0)$(tput setab 9) Installation de INXI $(tput sgr0)" &&
echo ""
sleep 1 &&

sudo apt install inxi -y

echo ""
echo "                      ✅ OK ✅" &&
sleep 0.5 &&


echo ""
echo "                $(tput setaf 0)$(tput setab 9) Installation de NMON $(tput sgr0)" &&
echo ""
sleep 1 &&

sudo apt install nmon -y

echo ""
echo "                      ✅ OK ✅" &&
sleep 0.5 &&

echo ""
echo "             $(tput setaf 0)$(tput setab 9) Installation de IPTRAF $(tput sgr0)" &&
echo ""
sleep 1 &&

sudo apt install iptraf -y

echo ""
echo "                      ✅ OK ✅" &&
sleep 0.5 &&
echo ""
echo "               $(tput setaf 0)$(tput setab 9) Installation de ACPI $(tput sgr0)" &&
echo ""
sleep 1 &&

sudo apt install acpi -y

echo ""
echo "                      ✅ OK ✅" &&
sleep 0.5 &&

echo ""
echo "               $(tput setaf 0)$(tput setab 9) Installation de S-Tui $(tput sgr0)" &&
echo ""
sleep 1 &&

sudo apt install s-tui stress -y

echo ""
echo "                      ✅ OK ✅" &&
sleep 0.5 &&

echo ""
echo "             $(tput setaf 0)$(tput setab 9) Installation de Phoronix $(tput sgr0)" &&
echo ""
sleep 1 &&

cd ~/Maintenance/ &&
sudo ./phoronix.sh -y

echo ""
echo "                      ✅ OK ✅" &&
sleep 0.5 &&


echo ""
echo ""
echo ""
echo "    $(tput setaf 1)$(tput setab 7) L'installation a été effectuée avec succès $(tput sgr0)" &&
sleep 0.5 &&

echo ""
echo ""
echo "                       👍👍👍"
sleep 4.5 &&
echo ""
echo ""
